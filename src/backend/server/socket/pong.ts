...
import { isAllowedToJoin, removePrivateGame } from "./privateGameAllowlist.js";
...
export function init_pong(io: SocketIOServer, fastify: FastifyInstance) {
    ...
    // Private invite lobbies: inviteId → first socket waiting for their opponent
    const privateLobbies = new Map<string, PongSocket>();
    // Tracks concluded private games to block re-joining
    const finishedPrivateGames = new Set<string>();
    ...
    pong.on("connection", (socket) => {
        ...
        socket.on("match.join", async (payload?: { inviteId?: string }) => {
            ...
            if (inviteId) {
                ...
                if (!isAllowedToJoin(inviteId, userId)) {
                    socket.emit("match.error", { message: "You are not invited to this game." });
                    return;
                }

                // Block re-joining a finished private game
                if (finishedPrivateGames.has(inviteId)) {
                    socket.emit("match.error", { message: "This game has already finished." });
                    return;
                }

                // Reconnect to an ongoing private match
                if (matches.has(inviteId)) {
                    const match = matches.get(inviteId)!;
                    const side = match.leftProfile.id === userId ? "left" : "right";
                    const isDisconnectedSide =
                        side === "left"
                            ? match.leftDisconnectedAt !== null
                            : match.rightDisconnectedAt !== null;
                    if (isDisconnectedSide) {
                        await handleReconnect(socket, match, userId);
                    }
                    return;
                }

                // First player: wait in the private lobby
        ...
        socket.on("disconnect", (reason) => {
            ...
            for (const [id, s] of privateLobbies.entries()) {
                if (s.id === socket.id) { privateLobbies.delete(id); break; }
            }
    ...
    async function createMatch(leftSocket: PongSocket, rightSocket: PongSocket, privateId?: string) {
        const id = privateId ?? nextMatchId();
