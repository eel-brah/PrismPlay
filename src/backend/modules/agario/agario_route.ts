...
export async function agario_routes(server: FastifyInstance) {
  server.get<{ Querystring: PlayersHistoryQuery }>(
    "/history/players",
    ...
    async (req) => {
      ...
      return listPlayerHistoryDb(Number(q.userId), q.take, q.skip);
    },
  );
  server.get<{ Params: RoomParams }>(
    "/history/rooms/:roomId",
    { preHandler: [server.auth] },
    async (req, res) => {
      const { roomId } = req.params;
      const room = await getRoomHistoryDb(Number(roomId));
      if (!room) return res.code(404).send({ message: "Room Not Found" });
      return room;
    },
  );
  server.get<{ Querystring: RoomsHistoryQuery }>(
    ...
    async (req) => {
      const q = req.query;
      return listRoomsHistoryDb(q.take, q.skip, q.onlyEnded);
    },
  );
  server.get(
    "/history/rooms/:roomId/leaderboard",
    { preHandler: [server.auth] },
    async (req, reply) => {
